...
C<fuel>, C<speed>, and C<max_speed>. The attributes are attached to the class,
and if there are several instances (objects) of the class then each will
store its own version of these variables.

(C<max_speed> is a lexical accessor, so cannot directly be accessed outside
the curly braces where it was declared. Lexical accessors are created with
L<Lexical::Accessor>, and are not currently a Moops built-in feature.)

Unlike other programming languages, Perl has no native concept of attributes.
The C<has> keyword instead sets up some extra methods for your class, called
"accessors", which allow you to access the data for the attributes.
