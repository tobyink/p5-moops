...
Note that the C<override> keyword is only available within classes declared
using L<Moose>. (Moose is a Perl OO framework.) It's still possible to
override methods without Moose, using the C<method> keyword:
...
The advantage of the C<override> keyword is that on installation, it checks
to make sure a method of the same name actually exists in the parent class,
helping avoid typos.
